...
# begin properties
properties() { '
kernel.string=NetHunter Kernel for Mi A3 by CaliBerrr @ github
do.devicecheck=1 #Use value 1 while using boot-patcher standalone
do.modules=1
do.systemless=0#Never use this for NetHunter kernels as it prevents us from writing to /lib/modules
do.cleanup=0
do.cleanuponabort=0
device.name1=laurel_sporut
device.name2=laurus
device.name3=
device.name4=
device.name5=
